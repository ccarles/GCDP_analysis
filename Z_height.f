...
** This program calculates the evolution of element abundances trought a fine slice of the galactic plane

*It outputs Z_height.out, giving for each time step, withtin the given radius :
...
*play with rad to control the range of the slice : this is still experimental so 0.5 maybe ? 
*******************
...
*    gas particles.
           ...
           ngf=0
           do i=1,ng
                 distz=ABS(rs(i,3)-zcm)
                 dist=dsqrt((rs(i,1)-xcm)**2+(rs(i,2)-ycm)**2)
                 if(distz.LT.height1.AND.dist.LT.rad1) then
                     ngf=ngf+1
                     listg(ngf)=idg(i)
                     mgf(ngf)=mg(i)
                     mzHgf(ngf)=mzHg(i)
