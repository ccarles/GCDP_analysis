...
*******************
      ...
      dimension gasmass(0:nbin), sfmass(0:nbin)

* Arrays containing all particles.

      dimension rs(nmax,3), ms(nmax), rg(nmax,3), mg(nmax),
     +          ids(nmax), idg(nmax), idg_old(nmax)
...
*doing python later.
           ...
           do i=1,ng
                dist=dsqrt(rg(i,1)**2+rg(i,2)**2)
                index=int(dist/dr)+1
                gasmass(index)=gasmass(index)+mg(i)
           enddo
*********************
           ...
           do i=1,ns
                ...
                do j=1,ng_old
                     if(idg_old(j).eq.id_star) then
                          dist=dsqrt(rs(i,1)**2+rs(i,2)**2)
                          index=int(dist/dr)+1
                          sfmass(index)=sfmass(index)+ms(i)
 ...
 3000                     format(6(2x,1pe13.6))
    ...
    3      enddo
           ...
           do i=1,nbin
                ...
                write(12,6001) t, dist, dr, gasmass(i),
     +                         sfmass(i), 
     +                         sfr, ssfr, i
