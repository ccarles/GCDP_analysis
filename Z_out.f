...
*Last update : 16 agust 2017

** This program calculates the evolution of element abundances within a given radius, very large in this case..

*It outputs Z_global.out, giving for each time step, withtin the given radius :
...
*format : t, nsf, ngf, mzHgtot, mH_movein, mH_moveout, mH_SF, mH_SN, deltaH1, mzOgtot, mO_movein, mO_moveout, mO_SF, mO_SN, deltaO1, OH, mzFegtot, mFe_movein, mFe_moveout, mFe_SF, mFe_SN, deltaFe1, FeH, mg, mg_movein, mg_moveout, mg_SF, mg_SN, deltamg, msf

*This program is to be used with the following GCDP output ASCII files :
* tzstep.dat, gXXXXXX, sXXXXXX.

*play with rad to control the of the zone : 10 is usually higher than the galaxy radius. 
*******************
...
* Arrays containing particles in desiredr region.
...
* Open files.
      ...
      open(unit=4,file='Z_o.out',status='unknown')
...
*
           ...
           do i=1,ns
                ...
                if(dist.LT.rad2.AND.dist.GT.rad1) then
                     nsf=nsf+1
                     lists(nsf)=ids(i)
                     msf=msf+ms(i)
                endif
